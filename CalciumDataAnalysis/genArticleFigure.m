...
pixelPerUm = 0.3;  % enter a value if you wish to override the data taken from scanimage, or use '[]'
...
startOfScaleBar = floor(size(Cn, 1) * 0.03);
heightOfScaleBar = floor(size(Cn, 2) * 0.09);
numOfCells = size(Fd_or{1, 1}, 1);
numOfTimepoints = size(Fd_or{1, 1}, 2);
...
%% Add contours
plot_contours_mod(A(:,keep),Cn,options,0,[],Coor,1,find(keep));
figure; hold on;
offsets = [1:numOfCells]';
offsets = repmat(offsets, 1, numOfTimepoints);  % adding offsets to all traces so that they won't overlap
maxVal = max(Fd_or{1, 1}, [], 2);
minVal = min(Fd_or{1, 1}, [], 2);
plot(header(1).frameTimestamps_sec(1, 1:numOfTimepoints), (Fd_or{1, 1} + minVal)./maxVal + offsets, ...
    'LineWidth', 1);
grid off; box off;
