...
%support single file (gets vector instead of cell in 't' and 'y')
if iscell(T{1,'t'})
    t=cell2mat(T{1,'t'});
else
    t=T{1,'t'};
...
if ~isempty(rowsDiamIdx)
    %support single entry in RES file
    if iscell(T{rowsDiamIdx,'y'})
...
if ~isempty(rowsVelocityIdx)
    ...
    %support single entry in RES file
    if iscell(T{rowsVelocityIdx,'y'})
        y=cell2mat(T{rowsVelocityIdx,'y'});
    else
        y=T{rowsVelocityIdx,'y'};
    end
...
if ~isempty(rowsIntensityIdx)
    ...
    %support single entry in RES file
    if iscell(T{rowsIntensityIdx,'y'})
        y=cell2mat(T{rowsIntensityIdx,'y'});
    else
        y=T{rowsIntensityIdx,'y'};
    end
    y=hampel(y,4,2);
    plot(t,y)
...
if ~displayOnly %no need to get data and validate Xi limits
    ...
    %support for single file in RES
    if iscell(currentData.t)
    t = currentData.t{:};
    else
        t = currentData.t;
    end
...
%figure;plot(t_selected);pause;close(gcf); used this line for testing
%selection.
for iROW = 1 : numel(rowIds);
    RES_FILES_COMPILED.t_selected{rowIds(iROW)} = t_selected;
...
if ~exist('RES_FILES_COMPILED','var')
    %this can be a single (not compiled, i.e. ALL_RES_FILE generated by
    %gatherRESfiles). Deal with loaded variables in workspace
    RES_FILES_COMPILED  = getWorkspaceVars(fullfile(path2ResFile,RESFileName),'all');
    [RES_FILES_COMPILED.RESfileName] = deal(RESFileName);
    %append dummy fields 'animalID','conditionID'
    [RES_FILES_COMPILED.animalID] = deal({NaN});
    [RES_FILES_COMPILED.conditionID] = deal('Null');
end
...
if ~ismember('t_selected',RES_FILES_COMPILED.Properties.VariableNames)
    ...
    for iFILE = 1 : max(inRESfileNamesIDs)
        ...
        %if the  file contains only one RES, we get a vector of doubles
        if iscell(RES_FILES_COMPILED{iRow(1),'t'})
            nPoints = numel(RES_FILES_COMPILED{iRow(1),'t'}{1});
        else
            nPoints = numel(RES_FILES_COMPILED{iRow(1),'t'});
        end
