...
% Last Modified by GUIDE v2.5 03-May-2011 18:22:03
...
function pushButtonLoadDataMat_Callback(hObject, eventdata, handles)
    ...
    fileExt = handles.fileNameMat(end-3:end);       % make sure selected file was right type
    if ~strcmpi(fileExt,'.mat')      
        % not an .mat file
        errordlg('Oops - needs to be an .mat (MATLAB) file')
        return
    end
    
    set(handles.figure1,'Name',['pathAnalyzeGUI     ' handles.fileNameMat '     ' handles.fileNameMpd]);
...
function pushButtonLoadDataMpd_Callback(hObject, eventdata, handles)
    [handles.fileNameMpd,handles.fileDirectory] = uigetfile([handles.fileDirectory '*.mpd'],'open file - MpScan (*.mpd)'); % open file
    % MATEO 20120313
    MATEO_MPD_NAME=fullfile (handles.fileDirectory,handles.fileNameMpd); display (MATEO_MPD_NAME)
    % END MATEO 20120313
    if isequal(handles.fileNameMpd,0)   % check to make sure a file was selected
        return
    end
    
    fileExt = handles.fileNameMpd(end-3:end);
    
    if ~strcmpi(fileExt,'.mpd')        
        % not an .mpdf file
...
function pushButtonInitialize_Callback(hObject, eventdata, handles)
    ...
    handles.scanDataLines100 = ... 
        mpdRead([handles.fileDirectory handles.fileNameMpd],'lines',handles.imageCh,1:100);
    ...
    % if they differ, just use the one in the MPD
...
function pushButtonDiameterTransform_Callback(hObject, eventdata, handles)
    ...
    dataStruct = struct( ...
        'fullFileNameMpd',[handles.fileDirectory handles.fileNameMpd], ... 
...
function writeForLater(dataStruct,handles)
    ...
    escapedFilename = regexprep(dataStruct.fullFileNameMpd,'\\','\\\');  % changes \ to \\
    
    fprintf(fid,'dataStruct = struct( ...\n');
    fprintf(fid,[' ''fullFileNameMpd'',' '''' escapedFilename '''' ', ...\n'] ,'char');
