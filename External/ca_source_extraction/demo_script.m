...
addpath('/data/MatlabCode/PBLabToolkit/ScanImageUtils','-end');
% addpath('/data/Amos/GitHub/cvx');
% addpath('/data/Amos/GitHub/cvx/lib');
%nam = 'demoMovie.tif';          % insert path to tiff stack here
% sframe=1;						% user input: first frame to read (optional, default 1)
% num2read=3000;%length(imfinfo(nam));%2000; user input: how many frames to read   (optional, default until the end)
%Y = bigread2(nam,sframe,num2read);
[~,Y]=opentif(nam,'channel',1);%scanimage.util.opentif(nam,'channel',1);
%opentif retuns a 5D file, redundant here so squeeze to get MxNxF where F is number of frames in stack



%AG added. it avereages every 5 frames into one. this reduces memory load
% Frames=1;NewFrameCounter=1;MeanFrame=[];
% while Frames < size(Y,3)-4 
%  MeanFrame(:,:,NewFrameCounter)=mean(Y(:,:,Frames:Frames+4));
%  Frames=Frames+5; NewFrameCounter=NewFrameCounter+1;
% end
% Y=MeanFrame;
...
fps=30;
ChangeFpS=input('is the frame rate different from 30 frame/sec? \n   1 for yes (it is NOT 30) ;  0 for it is 30 \n');
if ChangeFpS
...
refine_components = 1;%false;  % flag for manual refinement
...
%% An example movie of merging. AG cancelled
display_merging = 0;%1; % flag for displaying merging example
